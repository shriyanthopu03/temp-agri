...
function SignUpScreen({ onLogin, onSignUp }) {
  ...
  const submit = async (event) => {
    ...
    setLoading(true)
    try {
      const data = await requestAuth('register', { name, email, password, role: accountType, organizationId: defaultOrganizationId, regionId: defaultRegionId })
      onSignUp(data)
    } catch (authError) {
      setError(authError.message)
    } finally {
...
export default function App() {
  ...
  return <div className="min-h-screen bg-background text-foreground">
    ...
    <main className="lg:pl-64"><header className="flex h-20 items-center justify-between border-b border-border bg-card/80 px-5 backdrop-blur md:px-8"><div className="flex items-center gap-3"><button className="lg:hidden" onClick={() => setMenuOpen(true)} aria-label="Open menu"><Menu size={22} /></button><div className="hidden items-center gap-2 text-sm text-muted-foreground md:flex"><span>Workspace</span><span>/</span><span className="font-medium text-foreground">{active}</span></div><div className="flex items-center gap-2 md:hidden"><Leaf size={18} className="text-primary" /><span className="font-semibold">AgriTrade</span></div></div><div className="flex items-center gap-3"><button className="hidden rounded-xl p-2.5 text-muted-foreground hover:bg-muted sm:block" aria-label="Search"><Search size={19} /></button><button className="relative rounded-xl p-2.5 text-muted-foreground hover:bg-muted" aria-label="Notifications"><Bell size={19} /><span className="absolute right-2 top-2 size-1.5 rounded-full bg-accent" /></button><div className="hidden h-7 w-px bg-border sm:block" /><div className="relative"><button className="flex items-center gap-2" onClick={() => setAccountMenuOpen((open) => !open)} aria-expanded={accountMenuOpen} aria-haspopup="menu"><div className="flex size-9 items-center justify-center rounded-full bg-secondary text-sm font-semibold text-primary">{userInitials}</div><div className="hidden text-left md:block"><p className="text-sm font-semibold">{userName}</p><p className="text-xs text-muted-foreground">{account.label}</p></div><ChevronDown size={15} className={`hidden text-muted-foreground transition-transform md:block ${accountMenuOpen ? 'rotate-180' : ''}`} /></button>{accountMenuOpen && <div className="absolute right-0 top-12 z-30 w-52 rounded-xl border border-border bg-card p-2 shadow-lg" role="menu"><button className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm text-foreground hover:bg-muted" onClick={() => setDarkMode((dark) => !dark)} role="menuitem">{darkMode ? <Sun size={17} /> : <Moon size={17} />} {darkMode ? 'Light mode' : 'Dark mode'}</button><button className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm text-foreground hover:bg-muted" onClick={logout} role="menuitem"><LogOut size={17} /> Log out</button></div>}</div></div></header>
      <div className="mx-auto max-w-[1500px] p-5 md:p-8"><WorkspacePanel active={active} farms={farms} /><div className="mt-8 mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="mb-2 flex items-center gap-2 text-sm font-medium text-primary"><CloudSun size={17} /> Thursday, 05 September 2026</p><h1 className="text-3xl font-semibold tracking-tight md:text-4xl">Good morning, {userName}</h1><p className="mt-2 text-muted-foreground">Here is what is happening across your farms today.</p></div><button onClick={() => setShowForm(true)} className="flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground shadow-sm transition hover:opacity-90"><Plus size={18} /> Add new farm</button></div>
